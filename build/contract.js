...
let FungibleToken = (_dec = NearBindgen({
...
}), _dec4 = call({}), _dec5 = call({}), _dec6 = view({}), _dec7 = view({}), _dec(_class = (_class2 = class FungibleToken {
  ...
  internalUnregisterAccount({
    ...
    log("Unregistered account " + accountId + " and refunded " + deposit + " yoctoNEAR to " + registrantAccountId);
  }
  ...
  internalDeposit({
    ...
    let balance = this.internalGetBalance({
      accountId
    });
  ...
  internalTransfer(senderId, receiverId, amount, memo = null) {
    ...
    const remainingBalance = this.internalGetBalance({
      accountId: senderId
    });

    if (remainingBalance === "0") {
      this.internalUnregisterAccount({
        accountId: senderId
      });
    }
  }
